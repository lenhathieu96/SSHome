...
export default function HomeScreen({navigation}) {
  ...
  useEffect(() => {
    if (hardware.WFEnabled && userRole && userRole !== 'Master') {
      getMemberData();
      if (homeID && memberID) {
        let userRooms = [];
        firestore()
          ...
          .onSnapshot((UserData) => {
            if (UserData.data()) {
              userRooms = UserData.data().availableRooms;
              let data = {
                id: UserData.id,
          ...
          .on('value', (snapshot) => {
            ...
            userAvailableRooms.forEach((roomID) => {
              for (const room in snapshot.val()) {
                if (room === roomID) {
                  result.push(snapshot.val()[room]);
                }
              }
            });
            dispatch(setAvailableRoom(result));
          });
        return () => database().ref(homeID).off('value', roomSubsribe);
      }
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
      ...
      if (roleStorage) {
        const masterID =
          roleStorage === 'Master'
            ...
            : await AsyncStorage.getItem('@masterID');
        if (masterID) {
          const response = await findRealRoomID(masterID);
  ...
  const getMasterData = async () => {
    ...
    if (response && response.result) {
      dispatch(setUserProfile(response.data.userInfo));
    }
  };
